...
```r
library(TeachingSampling)library(samplesize4surveys)library(stratification)setwd("/. . ./ Your Folder")load("VotaPer. Rdata")load("VotaMuni. Rdata")
```
...
sam2p <- ss2s4p(N, p = mean(VotaPer$santos), conf = 0.95, me = 0.03, M = 200,by = 5, rho = rho_santos)View(sam2p)
```
...
VotaMuni$total <- rowSums(VotaMuni[,3:14])sum(VotaMuni$total)
res <- S. STpiPS(VotaMuni$H, VotaMuni$total, nh)
sam <- res[,1]sample <- VotaMuni[sam,]View(sample)
```
