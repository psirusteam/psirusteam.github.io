...
$X_1 \sim \operatorname{Binom}(n, p)$

Por supuesto, $p=\Pr(T=1)$ representa la probabilidad de que el resultado de la prueba sea positivo. Ahora, nótese que $X_2 \sim \operatorname{Binom}(n, 1-p)$ y que *p* puede ser factorizado de la siguiente manera:

$p=\Pr(T=1)=\Pr(T=1\mid D=1)\Pr(D=1)+ \Pr(T=1\mid D=0)\Pr(D=0)=\eta\pi+(1-\theta)(1-\pi)$
...
$\Pr(D=1\mid T=1)=\Pr(T=1\mid D=1)\Pr(D=1)/\Pr(T=1)=\frac{\eta\pi}{p}$

Por lo tanto

$Z_1\mid X_1 \sim \operatorname{Binom}(X_1, \frac{\eta\pi}{p})$
...
$\Pr(D=1\mid T=0)=\Pr(T=0\mid D=1)\Pr(D=1)/\Pr(T=0)=\frac{(1-\eta)\pi}{1-p}$

Entonces, fácilmente se concluye que

$Z_2\mid X_2 \sim \operatorname{Binom}(X_2, \frac{(1-\eta)\pi}{1-p})$
...
$\Pr(T=1, D=1)=\Pr(T=1\mid D=1)\Pr(D=1)=\eta\pi$

$\Pr(T=0, D=1)=\Pr(T=0\mid D=1)\Pr(D=1)=(1-\eta)\pi$

$\Pr(T=1, D=0)=\Pr(T=1\mid D=0)\Pr(D=0)=(1-\theta)(1-\pi)$

$\Pr(T=0, D=0)=\Pr(T=0\mid D=0)\Pr(D=0)=\theta(1-\pi)$

Por lo tanto, la función de densidad multinomial para los datos observados y latentes mostrados en la tabla anterior es

$\ell(\mathbf{X}, \mathbf{Z}\mid\eta, \theta, \pi) \propto (\eta\pi)^{Z_1}((1-\eta) \pi)^{Z_2}((1-\theta)(1-\pi))^{ X_1-Z_1}(\theta(1-\pi))^{ X_2-Z_2}$
...
$\eta \sim \operatorname{Beta}(a_{\eta}, b_{\eta})$

$\theta \sim \operatorname{Beta}(a_{\theta}, b_{\theta})$

$\pi \sim \operatorname{Beta}(a_{\pi}, b_{\pi})$
...
$f(\eta, \theta, \pi\mid\mathbf{X}, \mathbf{Z}) \propto \pi^{Z_1+Z_2+a_{\pi}}(1-\pi)^{X_1+X_2-Z_1-Z_2+b_{\pi}}\eta^{Z_1+a_{\eta}}(1-\eta)^{Z_2+b_{\eta}} \theta^{X_2-Z_2+a_{\theta}}(1-\theta)^{X_1+Z_1+b_{\theta}}$
...
$\eta\mid Z_1, Z_2 \sim \operatorname{Beta}(Z_1+a_{\eta}, Z_2+b_{\eta})$
...
$\theta\mid X_1, X_2, Z_1, Z_2 \sim \operatorname{Beta}(X_2-Z_2+a_{\theta}, X_1-Z_1+b_{\theta})$

- Para el parámetro de prevalencia de la enfermada, se encuentra que el *kernel*de la distribución condicional posterior es

$\pi\mid X_1, X_2, Z_1, Z_2 \sim \operatorname{Beta}(Z_1+Z_2+a_{\pi}, X_1+X_2-Z_1-Z_2+b_{\pi})$
